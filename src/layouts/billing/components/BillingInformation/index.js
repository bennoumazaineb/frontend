...
function BillingInformation() {
  ...
  const isEmployee = currentUser?.role === "employee"; // Vérifier le rôle "employee"
  const userFullName = currentUser?.Nom_Prénom;

  const handleAddAvis = () => {
    navigate("/Ajouter_avis");
  };
      ...
      <MDBox pt={1} pb={2} px={2}>
        <MDBox component="ul" display="flex" flexDirection="column" p={0} m={0}>
          {allAvis.map((avis) => {
            ...
            if (isAdmin || (isPartner && affecte_par === userFullName) || (isClient && affecte_par === userFullName) || (isEmployee && affecte_par === userFullName)) {
              return (
                <Bill
                  key={_id}
                  _id={_id}
